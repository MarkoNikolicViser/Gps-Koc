...
const ListaKomentara = ({
  ...
  const BrisanjeKomentara = async () => {
    let result = window.confirm(
      `Da li želite da obrišete komentar '${info.komentar}'`
    );
    if (!result) return null;
    const data = await (
      await fetch(`${url}komentar/delete/${info.Id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <tr style={{ ...Boje(info.boja), overflowWrap: 'anywhere' }}>
      <td className='important'>{info.komentar}</td>
