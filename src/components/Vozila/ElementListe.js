...
const ElementListe = ({ vozila, setVozila, vozilo, bazaInfo }) => {
  ...
  const VratiNazivUredjaja = idHardvera => {
    wialon.core.Session.getInstance().getHwTypes(
      ...
      function (code, col) {
        ...
        col.map(m => {
          if (m.id === idHardvera) setNazivUredjaja(prev => (prev = m.name));
        });
      }
    );
  };
  const Senzor = nazivSenzora => {
    ...
    for (var i in sens) {
      if (sens[i].n.includes(nazivSenzora)) napon = sens[i].id;
    }
          ...
          <div style={ObojRed()} className='row'>
            ...
            >
              <div>
                <b
                  className='ime'
                  onClick={() => NameClick(setVoziloInfoValue)}
                >
                  {vozilo.name}
                </b>
            ...
            >
              ...
              >
                ...
                {Senzor('Brzina') != 'N/A' ? (
                  ...
                  <h5>{vozilo.raw.$$user_position.s.toFixed(0)} km/h</h5>
                )}
              </div>
            </div>
