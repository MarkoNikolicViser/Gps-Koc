  ...
  const PosaljiUpit = async e => {
    ...
    try {
      ...
      if (data.length !== undefined) {
        ...
        ime = ime.replace('.', ' ');
        setKorisnikValue(
          prev => (prev = { Id: data[0].Id, ime: ime, pravo: data[0].pravo })
        );
      ...
      setLoginPoruka('Problem sa serverom, obratite se administratoru.');
    }
  };

  useEffect(() => {
    if (passOn) PassRef.current.type = 'text';
    else {
            ...
            <div className='sign-in-htm'>
              <form onSubmit={PosaljiUpit}>
                <div className='group'>
                  ...
                  <input
                    id='user'
                    onChange={e => setMail(prev => (prev = e.target.value))}
                    type='text'
                    className='input'
                  />
                </div>
                ...
                <div className='group'>
                  <h1 style={{ color: 'red' }}>{loginPoruka}</h1>
